...
class MountainCarQAgent():
    def __init__(self, buckets=(18, 14), episodes=10000, min_lr=0.1, min_epsilon=0.01, discount=1.0, decay=150):
        ...
        self.maxval = [self.env.observation_space.high[0], self.env.observation_space.high[1]]
        self.minval = [self.env.observation_space.low[0], self.env.observation_space.low[1]]
...
reward = agent.run()
print("Not trained agent ended with reward of", reward, sep=" ")
agent.train()
agent.env.close()
#%%
...
ax.grid(alpha = 0.5)
ax.set_title("Q-learning rewards during 5000 episodes")
fig.show()
#%%
reward = agent.run()
print("Agent trained for", agent.episodes, "episodes gained reward of", reward, sep=" ")
agent.env.close()
# %%
